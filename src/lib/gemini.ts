...
export async function generateSouvenir(mood: string, reflection: string, language: 'en' | 'zh' = 'en'): Promise<GeneratedSouvenir> {
  ...
  const prompt = `
    ...
    EXCEPT for 'imageKeyword', which MUST ALWAYS be in ENGLISH.
    ...
    - imageKeyword: A SINGLE ENGLISH WORD representing the core object noun (e.g., 'ticket', 'coin', 'ring', 'letter', 'matchbook'). MUST BE IN ENGLISH.
  ...
  while (retries > 0) {
    try {
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            ...
            required: ["title", "place", "subtitle", "narrative", "interpretation", "moodTags", "imageKeyword"]
          }
        }
      });
      ...
      const souvenir: GeneratedSouvenir = {
        ...
        imageKeyword: parsed.imageKeyword || "object"
      };

      // Fetch a relevant image using a free public image API (Pollinations) to bypass Gemini quota limits
      try {
        // Use the single English keyword generated by Gemini to ensure a simple, reliable image generation
        const keyword = souvenir.imageKeyword || "object";
        
        // Use Pollinations.ai with just the keyword to prevent timeouts and parsing errors
        souvenir.imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(keyword)}?width=1600&height=900&nologo=true`;
      } catch (imageError: any) {
        console.error("Image URL construction failed:", imageError);
